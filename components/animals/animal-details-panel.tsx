...
import { Battery, Clock, MapPin, Satellite, Trash2, X } from 'lucide-react'
...
export function AnimalDetailsPanel() {
	...
	const isPointInZone = (point: { lat: number; lng: number }, zoneCoords: { lat: number; lng: number }[]) => {
		...
		for (let i = 0, j = zoneCoords.length - 1; i < zoneCoords.length; j = i++) {
			const xi = zoneCoords[i].lng,
				yi = zoneCoords[i].lat
			const xj = zoneCoords[j].lng,
				yj = zoneCoords[j].lat
	...
	const currentZone = animal.coordinates ? zones.find(z => isPointInZone(animal.coordinates!, z.coordinates)) : null
			...
			<CardContent className='space-y-4'>
				...
				<div className='grid grid-cols-2 gap-3'>
					<div className='flex items-center gap-2 rounded-lg bg-muted/50 p-2'>
						<Battery
							className={`h-4 w-4 ${
								animal.batteryLevel > 50
									? 'text-green-500'
									: animal.batteryLevel > 20
										? 'text-yellow-500'
										: 'text-red-500'
							}`}
						/>
					...
					<div className='flex items-center gap-2 rounded-lg bg-muted/50 p-2'>
						...
						<div>
							...
							<p className='font-medium text-sm'>
								{animal.hasGpsSignal ? `${animal.satellites} спутн.` : 'Нет сигнала'}
							</p>
						</div>
					</div>
				</div>
