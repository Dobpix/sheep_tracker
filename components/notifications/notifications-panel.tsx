...
import { Bell, X, AlertTriangle, WifiOff, Battery } from 'lucide-react'
  ...
  useEffect(() => {
    notifications.forEach(notif => {
      if (!notif.read && !shownNotifications.has(notif.id)) {
        const icon = notif.type === 'zone_exit' 
  ...
  return (
    <>
      ...
      <Button
        variant="outline"
        size="icon"
        className="absolute right-4 bottom-4 z-[1000] h-12 w-12 rounded-full bg-card shadow-lg"
      ...
      {/* Panel */}
      {isOpen && (
        <Card className="absolute right-4 bottom-20 z-[1000] w-80 bg-card/95 backdrop-blur-sm shadow-xl">
